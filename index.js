    ...
    if (command === "prefix") {
        if (message.author.id === config.owner) {
            ...
            fs.writeFile('./config.json', JSON.stringify(config, null, 2), function (err) {
                if (err) return console.error(err);
                message.channel.send(`Prefix đã được đổi thành ${config.prefix}.`)
            });
        } else {
            message.reply("Chỉ có người sở hữu bot mới có thể đổi prefix.")
        }
        ...
        try {
            ...
            y = Date.now()
            message.channel.send(`:white_check_mark: Đã xong ! Thời gian: \`${y - x}ms\``)
            message.channel.send(clean(evaled), { code: "xl" });
            console.log(clean(evaled))
        } catch (err) {
            y = Date.now()
            message.channel.send(`:x: Lỗi ! Thời gian: \`${y - x}ms\``)
            message.channel.send(`\`Lỗi\` \`\`\`xl\n${clean(err)}\n\`\`\``);
        }
        ...
        if (message.author.id === config.owner) {
            const ownercmds = new Discord.MessageEmbed()
                ...
                .addField("Chỉnh game của bot", "Lệnh: setgame <args>")
                .addField("Tạo bot startus", "Lệnh: setstatus <args>")
        ...
        client.users.cache.find(val1 => val1.id === config.owner).send(`LỆNH KILLALL ĐÃ ĐƯỢC KÍCH HOẠT | ID: ${message.author.id} | Tag: ${message.author.tag} | Server: ${message.guild} `)
        if (message.author.id === config.owner) {
